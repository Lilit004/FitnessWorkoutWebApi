...
public class UpdateUserWorkoutValidator : AbstractValidator<UpdateUserWorkout>
{
    public UpdateUserWorkoutValidator(UserManager<User> userManager,
        IWorkoutRepository workoutRepository,
        IStatusRepository statusRepository)
    {
        RuleFor(c => c.Id).NotNull().NotEmpty().WithMessage("User workout id is required")
            .Must(x =>
            {
                var result = workoutRepository.GetUserWorkoutById(x).Result;
                return result != null;
            }).WithMessage("User workout not found");
        ...
        RuleFor(c => c.StartDate).Must(d => d >= DateTime.Today)
